...
echo "== statusline 上下文用量段 =="
out_ctx="$(printf '%s' '{"model":{"display_name":"X"},"context_window":{"used_percentage":72,"context_window_size":200000}}' | bash "$SL")"
ok "ctx 出百分比"        "printf '%s' \"\$out_ctx\" | grep -q '72%'"
ok "ctx 出进度条 ▓"      "printf '%s' \"\$out_ctx\" | grep -q '▓'"
ok "ctx 出 /200k 分母"   "printf '%s' \"\$out_ctx\" | grep -q '/200k'"
out_ctxhi="$(printf '%s' '{"model":{"display_name":"X"},"context_window":{"used_percentage":90,"context_window_size":200000}}' | bash "$SL")"
ok "ctx 高位出 ⚠ /compact" "printf '%s' \"\$out_ctxhi\" | grep -q '/compact'"
out_ctx0="$(printf '%s' '{"model":{"display_name":"X"},"context_window":{"used_percentage":0}}' | bash "$SL")"
ok "ctx 0% 不被丢掉"     "printf '%s' \"\$out_ctx0\" | grep -q '0%'"
out_noctx="$(printf '%s' '{"model":{"display_name":"X"}}' | bash "$SL")"
ok "无 context 字段时不出 ctx 段" "! printf '%s' \"\$out_noctx\" | grep -q 'ctx '"
